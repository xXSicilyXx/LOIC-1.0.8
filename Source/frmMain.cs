	...
	public partial class frmMain : Form
	{
		...
		private void LockOnURL(bool silent = false)
		{
			try
			{
				...
				if (!tURL.Contains("://"))
				{
					if (tURL.StartsWith("https://")) tURL = String.Concat("https://", tURL);
					else tURL = String.Concat("http://", tURL);
				}
				try
				{
		...
		private void DoHive(bool enabled)
		{
			try
			{
				if ((txtIRCserver.Text == "" || txtIRCchannel.Text == "") && enabled) disableHive.Checked = true;
				else if (enabled)
				{
